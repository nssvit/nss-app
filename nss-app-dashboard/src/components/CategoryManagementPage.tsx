...
function CategoryModal({ isOpen, onClose, onSubmit, initialData, mode }: CategoryModalProps) {
  ...
  const handleCategoryNameChange = (value: string) => {
    setFormData(prev => ({
      ...prev,
      category_name: value,
      code: mode === "create" ? value.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') : prev.code,
    }));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              disabled={mode === "edit"}
            />
            <p className="text-xs text-gray-500 mt-1">Lowercase with hyphens, auto-generated from name</p>
          </div>
...
export function CategoryManagementPage() {
  ...
  const handleDeleteCategory = async (category: CategoryWithStats) => {
    ...
    if (!confirm(`Are you sure you want to permanently delete "${category.category_name}"? This cannot be undone.`)) {
      return;
    }
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchCategories} className="mt-4 button-glass-primary px-4 py-2 rounded-lg">
            Retry
          </button>
        </div>
      </div>
    ...
    <div className={`flex-1 overflow-x-hidden overflow-y-auto main-content-bg mobile-scroll safe-area-bottom ${layout.getContentPadding()}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={fetchCategories} className="action-button text-gray-400 hover:text-gray-200 p-2 rounded-lg">
      ...
      {/* Categories Grid */}
      <div className={`grid ${layout.isMobile ? "grid-cols-1" : "grid-cols-2 lg:grid-cols-3"} gap-4`}>
        {loading ? (
          ...
          filteredCategories.map((category) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <div
                    className="w-10 h-10 rounded-lg flex items-center justify-center bg-indigo-900/30"
                  >
