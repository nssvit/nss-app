...
export function EventParticipantsModal({
  ...
  const filteredParticipants = participants.filter((p) => {
    const matchesSearch =
      `${p.first_name} ${p.last_name} ${p.roll_number} ${p.email}`
        .toLowerCase()
  ...
  const handleExportCSV = () => {
    const headers = ["Name", "Roll Number", "Branch", "Year", "Email", "Status", "Hours", "Registration Date"];
                ...
                >
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded-full ${
                      STATUS_COLORS[participant.participation_status] || "text-gray-400 bg-gray-700/30"
                    }`}
                  >
