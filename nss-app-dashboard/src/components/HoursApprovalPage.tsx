...
import { useHoursApproval, ParticipationForApproval, ApprovalFilter } from "@/hooks/useHoursApproval";
    ...
    >
      <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 p-6 rounded-2xl shadow-2xl w-full max-w-lg">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-white text-2xl leading-none p-1">
            ×
          </button>
        </div>
        ...
        <div className="mb-6 p-4 bg-gray-800/30 rounded-lg">
          <div className="flex items-center space-x-3 mb-3">
            <Image
              ...
              alt={participation.volunteer ? `${participation.volunteer.first_name} ${participation.volunteer.last_name}` : "Unknown"}
              width={40}
              height={40}
            ...
            <div>
              <h4 className="font-medium text-gray-200">
                {participation.volunteer ? `${participation.volunteer.first_name} ${participation.volunteer.last_name}` : "Unknown"}
              </h4>
              <p className="text-sm text-gray-400">{participation.volunteer?.roll_number}</p>
            </div>
          </div>
          <div className="text-sm text-gray-400">
            <p><strong>Event:</strong> {participation.event?.event_name || "Unknown"}</p>
            <p><strong>Date:</strong> {participation.event?.start_date ? new Date(participation.event.start_date).toLocaleDateString() : "N/A"}</p>
            <p><strong>Requested Hours:</strong> {participation.hours_attended}</p>
          </div>
        </div>
...
export function HoursApprovalPage() {
  ...
  const [selectedParticipation, setSelectedParticipation] = useState<ParticipationForApproval | null>(null);
  ...
  const handleSelectAll = () => {
    ...
    setSelectedItems(
      selectedItems.length === pendingItems.length ? [] : pendingItems
    );
  };
  ...
  const handleModalSubmit = async (hours: number, notes: string) => {
    ...
    if (modalMode === "approve") {
      const result = await approveHours(
      ...
      const result = await rejectHours(
        selectedParticipation.id,
        currentUserId,
        notes
      );
  ...
  const handleBulkApprove = async () => {
    ...
    const result = await bulkApproveHours(selectedItems, currentUserId);
    if (result.error) {
      showToast(result.error, "error");
    } else {
      showToast(`${result.count} records approved`, "success");
      setSelectedItems([]);
    }
  };
    ...
    >
      ...
      <div className="card-glass rounded-xl overflow-hidden">
        ...
        <div className="bg-gray-800/30 px-4 py-3 border-b border-gray-700/30">
          <div
            className={`grid ${
              layout.isMobile ? "grid-cols-1" : "grid-cols-7"
            } gap-4 items-center`}
          >
            {!layout.isMobile && (
              <>
