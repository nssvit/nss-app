...
export function ReportsPage() {
  ...
  const topEvents = [
    { name: "Blood Donation Drive", participants: 118, hours: 354, impact: "High" },
    { name: "NSS Camp - Kuderan", participants: 48, hours: 2400, impact: "Very High" },
    { name: "Beach Clean-Up", participants: 73, hours: 292, impact: "High" },
    { name: "Digital Literacy Workshop", participants: 32, hours: 192, impact: "Medium" },
        ...
        {metrics.map((metric, index) => (
          <div key={index} className="card-glass rounded-xl p-4 md:p-5">
            <div className="flex items-center justify-between mb-3">
              <div className={`w-12 h-12 rounded-lg bg-gray-800/50 flex items-center justify-center ${metric.color}`}>
                <i className={`${metric.icon} text-lg`}></i>
              </div>
              <div className={`flex items-center space-x-1 text-sm ${
                metric.changeType === 'increase' ? 'text-green-400' : 'text-red-400'
              }`}>
                <i className={`fas fa-arrow-${metric.changeType === 'increase' ? 'up' : 'down'} text-xs`}></i>
