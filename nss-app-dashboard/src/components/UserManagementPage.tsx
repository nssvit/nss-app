      ...
      <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 p-6 rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {roles.map((role) => (
                <span key={role} className="text-xs px-2 py-1 rounded-full bg-blue-900/30 text-blue-400">
                  {role}
                </span>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="text-gray-200">{volunteer.roll_number}</p>
          </div>
          <div>
        ...
        <div className="flex justify-end mt-6">
          <button onClick={onClose} className="button-glass-secondary hover-lift px-4 py-2 text-sm rounded-lg">
            Close
          </button>
        </div>
      </div>
...
export function UserManagementPage() {
  ...
  const { volunteers, loading, error, fetchVolunteers, updateVolunteer, deleteVolunteer, adminPermanentDelete, checkIsAdmin } = useVolunteers();
  const { userRoles, getVolunteerRoles } = useRoles();
  ...
  // Get roles for a volunteer
  const getRolesForVolunteer = (volunteerId: string) => {
    const roles = getVolunteerRoles(volunteerId);
    return roles.map(r => r.role_definition?.display_name || r.role_definition?.role_name || "Unknown");
  };

  // Get primary role (highest hierarchy)
  const getPrimaryRole = (volunteerId: string) => {
    const roles = getVolunteerRoles(volunteerId);
    if (roles.length === 0) return "volunteer";
    // Sort by hierarchy level (lower = higher priority)
    const sorted = [...roles].sort((a, b) =>
      (a.role_definition?.hierarchy_level || 100) - (b.role_definition?.hierarchy_level || 100)
    );
    return sorted[0]?.role_definition?.role_name || "volunteer";
  };
  ...
  const handleToggleActive = async (volunteer: Volunteer) => {
    const action = volunteer.is_active ? "deactivate" : "reactivate";
    if (!confirm(`Are you sure you want to ${action} ${volunteer.first_name} ${volunteer.last_name}?`)) {
      return;
    }
  ...
  const handleBulkDeactivate = async () => {
    ...
    for (const userId of selectedUsers) {
      const result = await updateVolunteer(userId, { is_active: false });
      if (!result.error) successCount++;
    }

    showToast(`${successCount} users deactivated`, "success");
    setSelectedUsers([]);
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchVolunteers} className="mt-4 button-glass-primary px-4 py-2 rounded-lg">
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`flex-1 overflow-x-hidden overflow-y-auto main-content-bg mobile-scroll safe-area-bottom ${layout.getContentPadding()}`}>
      ...
      <div className={`flex flex-wrap items-center gap-3 mb-6 ${layout.isMobile ? "px-0" : "px-1"}`}>
        ...
        <button className="text-gray-500 hover:text-gray-300 text-sm py-2 px-3" onClick={clearFilters}>
          Clear
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button onClick={fetchVolunteers} className="action-button text-gray-400 hover:text-gray-200 p-2 rounded-lg">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex flex-col space-y-1">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${getStatusColor(volunteer.is_active)}`}>
                            {volunteer.is_active ? "Active" : "Inactive"}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex space-x-2">
                          <button onClick={() => handleEdit(volunteer)} className="text-gray-400 hover:text-blue-400 p-1">
                            <i className="fas fa-edit text-sm"></i>
                          </button>
                          <button onClick={() => handleView(volunteer)} className="text-gray-400 hover:text-green-400 p-1">
                            <i className="fas fa-eye text-sm"></i>
                          </button>
                          <button onClick={() => handleToggleActive(volunteer)} className="text-gray-400 hover:text-yellow-400 p-1">
                            <i className={`fas fa-${volunteer.is_active ? "ban" : "check-circle"} text-sm`}></i>
                          </button>
                          {isAdmin && (
                            <button onClick={() => handlePermanentDelete(volunteer)} className="text-gray-400 hover:text-red-600 p-1" title="Permanently Delete">
