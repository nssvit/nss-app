...
export function AttendanceManager() {
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    setParticipants((prev) => prev.map((p) => ({ ...p, selected: checked })))
  }

  const handleSelectParticipant = (participantId: string, checked: boolean) => {
    setParticipants((prev) =>
      prev.map((p) => (p.participant_id === participantId ? { ...p, selected: checked } : p))
    )
  }

  const handleStatusChange = (participantId: string, status: ParticipationStatus) => {
    setParticipants((prev) =>
      prev.map((p) => (p.participant_id === participantId ? { ...p, newStatus: status } : p))
    )
  }

  const handleHoursChange = (participantId: string, hours: number) => {
    setParticipants((prev) =>
      prev.map((p) => (p.participant_id === participantId ? { ...p, newHours: hours } : p))
    )
  }

  const handleNotesChange = (participantId: string, notes: string) => {
    setParticipants((prev) =>
      prev.map((p) => (p.participant_id === participantId ? { ...p, newNotes: notes } : p))
    )
  }
