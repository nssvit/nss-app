...
export function HoursApprovalPage() {
  ...
  const items = participations.map((p: any) => ({
    id: p.id,
    hoursAttended: p.hoursAttended || 0,
    ...
    volunteer: p.volunteer
      ? {
          ...
          rollNumber: p.volunteer.rollNumber || '',
          email: p.volunteer.email || '',
          profilePic: p.volunteer.profilePic,
        }
      : undefined,
    event: p.event
      ? { eventName: p.event.eventName || 'Unknown', startDate: p.event.startDate || '' }
      : undefined,
  }))
  ...
  const statsDisplay = [
    {
      ...
      f: 'pending' as ApprovalFilter,
    },
    {
      label: 'Approved',
      value: stats.approved,
      color: 'text-green-400',
      f: 'approved' as ApprovalFilter,
    },
    {
      label: 'Rejected',
      value: stats.rejected,
      color: 'text-red-400',
      f: 'rejected' as ApprovalFilter,
    },
    {
      label: 'Hours Pending',
      value: stats.totalHoursPending,
      color: 'text-purple-400',
      f: 'all' as ApprovalFilter,
    },
  ]
          ...
          >
            {!layout.isMobile ? (
              <>
                    ...
                    <input
                      ...
                      checked={
                        ...
                        selectedItems.length ===
                          filteredItems.filter((p) => p.approvalStatus === 'pending').length
                      }
                ...
                <div
                  className={`${filter === 'pending' ? 'col-span-2' : 'col-span-3'} text-sm font-medium text-gray-300`}
                >
                  Volunteer
                </div>
              ...
              <div className="text-sm font-medium text-gray-300">
                Hour Approvals ({filteredItems.length})
              </div>
            )}
          </div>
        </div>
        <HoursTable
          participations={filteredItems}
