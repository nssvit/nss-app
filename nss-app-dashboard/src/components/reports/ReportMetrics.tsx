      ...
      {metrics.map((metric, i) => (
        <div key={i} className="card-glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <span className={`${metric.color}`}><i className={`${metric.icon} text-lg`}></i></span>
            <span className={`text-xs px-2 py-1 rounded-full ${metric.changeType === 'increase' ? 'text-green-400 bg-green-900/30' : 'text-red-400 bg-red-900/30'}`}>
              {metric.change}
            </span>
          </div>
