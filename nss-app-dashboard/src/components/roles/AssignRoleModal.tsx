...
export function AssignRoleModal({
  isOpen, onClose, roleDefinitions, volunteers, existingRoles, onAssign,
  ...
  const volunteerHasRole = (volunteerId: string, roleId: string) =>
    existingRoles.some((ur) => ur.volunteer_id === volunteerId && ur.role_definition_id === roleId && ur.is_active)

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedVolunteer || !selectedRole || volunteerHasRole(selectedVolunteer, selectedRole)) return
    setSubmitting(true)
    try {
      ...
      <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 p-6 rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="text" placeholder="Search volunteers..." className="input-dark w-full text-sm rounded-lg px-4 py-2 mb-2"
              value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            <select required className="input-dark w-full text-sm rounded-lg px-4 py-3" value={selectedVolunteer}
          ...
          <div>
            ...
            <select required className="input-dark w-full text-sm rounded-lg px-4 py-3" value={selectedRole}
              onChange={(e) => setSelectedRole(e.target.value)}>
              <option value="">Select a role...</option>
              {roleDefinitions.filter((r) => r.is_active).map((role) => (
                <option key={role.id} value={role.id} disabled={selectedVolunteer && volunteerHasRole(selectedVolunteer, role.id)}>
          ...
          <div>
            ...
            <input type="date" className="input-dark w-full text-sm rounded-lg px-4 py-3" value={expiresAt}
              onChange={(e) => setExpiresAt(e.target.value)} min={new Date().toISOString().split('T')[0]} />
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button type="submit" disabled={submitting || !selectedVolunteer || !selectedRole}
