    ...
    <div className={`flex-1 overflow-x-hidden overflow-y-auto main-content-bg mobile-scroll safe-area-bottom ${layout.getContentPadding()}`}>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="relative flex-1 min-w-0">
          <input type="text" placeholder={activeTab === 'assignments' ? 'Search volunteers or roles...' : 'Search roles...'}
            className="input-dark text-sm rounded-lg py-2 px-3 pl-9 w-full" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        <button onClick={() => { setSearchTerm(''); setRoleFilter('') }} className="text-gray-500 hover:text-gray-300 text-sm py-2 px-3">Clear</button>
      </div>

      {/* Action Bar */}
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => activeTab === 'assignments' ? setShowAssignModal(true) : (setEditingRole(undefined), setShowRoleModal(true))}
        ...
        <button onClick={refetch} className="action-button text-gray-400 hover:text-gray-200 p-2 rounded-lg">
            ...
            {activeTab === 'assignments' ? (
              ...
              !layout.isMobile ? (
                <><div className="text-sm font-medium text-gray-300">Role Name</div>
                  ...
                  <div className="text-sm font-medium text-gray-300">Actions</div></>
              ) : <div className="text-sm font-medium text-gray-300">Role Definitions ({filteredRoles.length})</div>
            )}
          </div>
        </div>

        {activeTab === 'assignments' ? (
          <RoleAssignmentsTable userRoles={filteredUserRoles} loading={loading} isMobile={layout.isMobile} onRevoke={handleRevokeRole} />
        ) : (
          <RoleDefinitionsTable roles={filteredRoles} loading={loading} isMobile={layout.isMobile}
            onEdit={(role) => { setEditingRole(role); setShowRoleModal(true) }} onDeactivate={handleDeactivateRole} />
        )}
      </div>

      {/* Modals */}
      <AssignRoleModal isOpen={showAssignModal} onClose={() => setShowAssignModal(false)} roleDefinitions={roleDefinitions}
        volunteers={volunteers.map((v) => ({ id: v.id, first_name: v.firstName || '', last_name: v.lastName || '', email: v.email, profile_pic: v.profilePic || null }))}
        existingRoles={userRoles} onAssign={handleAssignRole} />
      <RoleDefinitionModal isOpen={showRoleModal} onClose={() => { setShowRoleModal(false); setEditingRole(undefined) }}
        onSubmit={handleCreateOrUpdateRole} initialData={editingRole} mode={editingRole ? 'edit' : 'create'} />
    </div>
  )
}
