import { pgTable, uuid, text, boolean, timestamp, date, integer, index, uniqueIndex, check } from 'drizzle-orm/pg-core'
...
export const volunteers = pgTable('volunteers', {
  ...
  check('volunteers_branch_check', sql`${table.branch} IN ('EXCS', 'CMPN', 'IT', 'BIO-MED', 'EXTC')`),
  check('volunteers_year_check', sql`${table.year} IN ('FE', 'SE', 'TE')`),
  check('volunteers_gender_check', sql`${table.gender} IS NULL OR ${table.gender} IN ('M', 'F', 'Prefer not to say')`),
])
