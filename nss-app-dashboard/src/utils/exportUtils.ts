...
export function exportAttendance(
  ...
  const formattedData = attendance.map((att) => ({
    ...
    registration_date: att.registration_date
      ...
      : '',
    attendance_date: att.attendance_date ? new Date(att.attendance_date).toLocaleDateString() : '',
    notes: att.notes || '',
  }))
...
export function exportHoursReport(data: HoursReportExport[], filename = 'nss_hours_report'): void {
