...
export const eventParticipation = pgTable(
  'event_participation',
  {
    ...
    recordedByVolunteerId: uuid('recorded_by_volunteer_id').references(() => volunteers.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    ...
    check(
      'participation_status_check',
      sql`${table.participationStatus} IN ('registered', 'present', 'absent', 'partially_present', 'excused')`
    ),
    check(
      'participation_approval_status_check',
      sql`${table.approvalStatus} IN ('pending', 'approved', 'rejected')`
    ),
    check(
      'participation_hours_check',
      sql`${table.hoursAttended} >= 0 AND ${table.hoursAttended} <= 24`
    ),
    check(
      'participation_approved_hours_check',
      sql`${table.approvedHours} IS NULL OR (${table.approvedHours} >= 0 AND ${table.approvedHours} <= 24)`
    ),
  ]
)
